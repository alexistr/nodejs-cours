        ...
        _data.read('users',userPhone, (err,userData) => {
          if(!err && userData) {
            ...
            if(userChecks.length < config.maxChecks) {
              // Verify that the URL given hsa DNS entries e(and therefore can be checked)
              ...
              dns.resolve(hostName, (err,records) => {
                if(!err && records) {
                  ...
                  callback(400,{'Error':'The hostname of the URL entered does not resolve to any DNS entries'});
                }
              });
