...
handlers._tokens.put = (data, callback) => {
  ...
  let password = typeof(data.payload.extend) == 'boolean' && data.payload.extend == true ? true : false;
  if(id && extend) {
    //lookup the token
    _data.read('tokens',id,(err,tokenData) => {
      if(!err && tokenData) {
        ...
        if(tokenData.expires > Date.now() {
          ...
          callback(404,{'Error': .'The token has already expires and can not be extended'})
        }
      } else {
        callback(404,{'Errore':'Specified token does not exists'});
      }
    })
  } else {
