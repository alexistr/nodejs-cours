      ...
      checks.forEach( (check) => {
        ...
        _data.read('checks',check,(err,originalCheckData) => {
          if(!err && originalCheckData) {
            ...
            debug("Error reading on of the check data." + err);
          }
        });
      });
  ...
  _data.update('checks',newCheckData.id,newCheckData,(err) => {
    if(!err) {
      ...
      if(alertWanted) {
        ...
        debug("Check stat has not changed no alert needed");
      }
    } else {
      debug("Error: trying to save updates of the checks");
    }
  });
...
workers.alertUserToSatusChange = (newCheckData) => {
  ...
  helpers.sendTwilioSms(newCheckData.userPhone,msg,(err)=>{
    if(!err) {
      debug('Success: user was alerted to a status change in their checks via sms: ' + msg);
    } else {
      debug('Error: could not send sms alert who had a state change in their state ');
    }
  });
};
          ...
          if(!err) {
            ...
            _logs.truncate(logId,(err) => {
              if(!err){
                ...
                debug("Error truncating one of the log file");
              }
            });
          } else {
